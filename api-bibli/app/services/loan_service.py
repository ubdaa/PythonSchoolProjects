...
class LoanService(TemplateService[Loan]):
    ...
    def _calculate_penalty(self, due_date: datetime, return_date: datetime) -> tuple[float, int]:
    ...
    def _enrich_loan(self, loan: Loan, book_title: str | None = None):
        ...
        if loan.status == LoanStatus.OVERDUE or (loan.return_date and loan.return_date > loan.due_date):
    ...
    async def create_loan(self, loan_data: LoanCreate) -> Loan:
        ...
        if book.available_copies <= 0:
            raise HTTPException(status_code=400, detail=f"Book '{book.title}' is not currently available")
        ...
        if active_loans >= MAX_LOANS_PER_USER:
            raise HTTPException(status_code=400, detail=f"Loan limit reached ({MAX_LOANS_PER_USER} max)")
    ...
    async def get_all_filtered(
        ...
        if active_only:
            statement = statement.where(or_(Loan.status == LoanStatus.ON_LOAN, Loan.status == LoanStatus.OVERDUE))
        if late_only:
